...
from app.models.models import Preset, Pipeline, User
...
async def test_load_builtin_presets(test_db, clean_presets):
    ...
    # Verify specific presets exist by name
...
@pytest.mark.asyncio
async def test_apply_preset_to_pipeline(test_db, clean_presets, test_user):
    ...
    # Verify edges generation
    assert len(pipeline.edges) == len(pipeline.nodes) - 1

@pytest.mark.asyncio
async def test_preset_api_endpoints(client: AsyncClient, test_db, clean_presets, auth_headers):
    ...
    response = await client.get("/api/v1/presets", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) >= 10
    
    # 2. GET /api/v1/presets/{id}
    response = await client.get(f"/api/v1/presets/{target_preset.id}", headers=auth_headers)
    assert response.status_code == 200
    data = response.json()
